...
last_status = {"steps": None, "distance": None, "time": None}
...
prom_state = Enum(
    "walkingpad_state",
    "WalkingPad state",
    states=["idle", "standby", "starting", "running", "stopping"],
)
prom_mode = Enum(
    "walkingpad_mode", "WalkingPad mode", states=["manual", "auto", "standby"]
)
...
def on_new_current_status(sender, record):
    ...
    if is_walking and (
        json_status["belt_state"] == "running"
        or json_status["belt_state"] == "starting"
    ):
...
async def on_web_request():
    ...
    if is_walking and (
        last_json_status["belt_state"] != "running"
        and last_json_status["belt_state"] != "starting"
    ):
        log.warning(
            "Automatically finishing walk since belt state is now {0}".format(
                last_json_status["belt_state"]
            )
        )
...
async def start_walk(request):
    ...
    try:
        ...
        )  # Ensure we start from a known state, since start_belt is actually toggle_belt
        await asyncio.sleep(minimal_cmd_space)
...
async def finish_walk(request):
    ...
    try:
        ...
        # await asyncio.sleep(minimal_cmd_space)
        store_in_db(
            session_current_steps - session_start_steps,
            session_current_dist - session_start_dist,
...
def auth_strava(request):
    ...
    return web.Response(
        text="<a href='{0}'>Authorize with Strava</a>".format(stravaUrl),
...
def strava_authorization(request):
    ...
    auth_response = stravaClient.exchange_code_for_token(
    ...
    return web.Response(
        text="<h1>Successfully authenticated with Strava!</h1>",
        content_type="text/html",
    )
...
def refresh_strava_token(client, config):
    ...
    if client.access_token is None or datetime.now() > datetime.fromtimestamp(
        expires_at
    ):
        log.info("Refreshing Strava client token...")
        try:
            refresh_response = client.refresh_access_token(
                MY_STRAVA_CLIENT_ID, MY_STRAVA_CLIENT_SECRET, refresh_token
            )
...
if __name__ == "__main__":
    config = load_config()
    access_token = (
        config.get("activity_trackers", {}).get("strava", {}).get("access_token", "")
    )
